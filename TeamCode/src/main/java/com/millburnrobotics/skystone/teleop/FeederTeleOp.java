...
public class FeederTeleOp extends OpMode {
    ...
    public void loop() {
        ...
        } else if (gamepad2.right_bumper && Robot.getInstance().getSideClaw().canToggleSideClaw()&& Robot.getInstance().getSideClaw().getClawPosition() > 0) {
            Robot.getInstance().getSideClaw().setClawPosition(Robot.getInstance().getSideClaw().getClawPosition() - SIDE_CLAW_INCREMENT);
        ...
        if (gamepad1.left_stick_button && Robot.getInstance().getChainBar().canToggleCapstone()){ // toggle foundation hook
            if (Robot.getInstance().getChainBar().getState() == ChainBar.CapstoneState.CAPSTONE_CLOSE) {
                Robot.getInstance().getChainBar().capstoneOpen();
            } else {
                Robot.getInstance().getChainBar().capstoneClose();
            }
        }
