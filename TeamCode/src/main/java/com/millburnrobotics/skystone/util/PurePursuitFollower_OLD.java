...
public class PurePursuitFollower_OLD {
    ...
    public PurePursuitFollower_OLD(Path path, double lookahead, double[] angles) {
        this.path = path;
    ...
    public PurePursuitFollower_OLD(Path path, double[] angles) {
        this.path = path;
        this.headingInterpolants = new double[path.getSegments().size()][2];
    ...
    public Vector2d reeeCollision(Pose2d currentPose) {
        ...
        return new Vector2d(Constants.DriveConstants.COLLISION_RECOVERY_MOVEMENT*Math.cos(deriv), Constants.DriveConstants.COLLISION_RECOVERY_MOVEMENT*Math.sin(deriv));
    }
}
