...
//        double maxPower = 0.85;
        ...
        runAction(new DriveRotationAction(90, 0.7)); // foundation
        runAction(new HookDownAction());
        timedAction(new DriveToPoseAction(new Pose(0, Robot.getInstance().getOdometry().getY(),3*Math.PI/2),0.5),300);
        timedAction(new DriveToPoseAction(new Pose(-72, Robot.getInstance().getOdometry().getY(),3*Math.PI/2),0.7),700);
