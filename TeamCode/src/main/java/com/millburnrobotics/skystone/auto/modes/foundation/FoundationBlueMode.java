...
public class FoundationBlueMode extends AutoModeBase {
    ...
    public void routine() {
        ...
        Robot.getInstance().getOdometry().setPose(new Pose(-63, 24+Constants.DriveConstants.BOT_WIDTH/2, Math.PI/2));

        runAction(new DriveToPoseAction(new Pose(-36,49,Math.PI/2), 0.5));
        ...
        timedAction(new DriveToPoseAction(new Pose(-66, 24+Constants.DriveConstants.BOT_WIDTH/2,Math.PI/2), 0.6), 3000);
        runAction(new DriveRotationAction(180, 0.5));

        runAction(new HookUpAction());
        runAction(new WaitAction(500));

        runAction(new DriveToPoseAction(new Pose(-63, 0, Math.PI/2), 0.5));
